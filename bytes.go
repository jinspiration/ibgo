...
func readLastTradeDate(resp *Message, c *ContractData) {
	if str := resp.readString(); str != "" {
...
func readFloat(buf *bytes.Buffer) float64 {
	s := readString(buf)
	f, err := strconv.ParseFloat(s, 10)
	if err != nil {
		panic(err)
...
func readIntUnset(buf *bytes.Buffer) int64 {
	s := readString(buf)
	if s == "" {
		return math.MaxInt64
	}
...
func readFloatUnset(buf *bytes.Buffer) float64 {
	s := readString(buf)
	if s == "" {
		return math.MaxFloat64
	}
...
func readStringList(buf *bytes.Buffer, sep string) []string {
	s := readString(buf)
	return strings.Split(s, sep)
}
...
func bytesToTime(b []byte) (time.Time, error) {
	...
	fmt.Println("bytetotime", string(b))
